...
public class ConsultaController {
    ...
    public ResponseEntity<ApiResponseDto<List<ConsultaDto>>> syncConsultas(
            @RequestBody List<ConsultaDto> consultas, HttpServletRequest request) {

        List<ConsultaDto> consultasDesdeServidor = consultaService.syncConsultas(consultas);

        return ResponseEntity.ok(
                ApiResponseDto.<List<ConsultaDto>>builder()
                        .status(HttpStatus.OK.value())
                        .message("Sincronización de consultas completada")
                        .timestamp(LocalDateTime.now())
                        .path(request.getRequestURI())
                        .data(consultasDesdeServidor)
                        .build()
        );
    }
}