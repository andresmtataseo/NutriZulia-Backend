...
public class EnfermedadController {
    ...
    @GetMapping(CATALOG_DISEASES)
    public ResponseEntity<ApiResponseDto<List<EnfermedadDto>>> getEnfermedades(
            HttpServletRequest request) {
        List<EnfermedadDto> enfermedadDtos =  enfermedadService.getEnfermedades();
        return ResponseEntity.ok(
                ApiResponseDto.<List<EnfermedadDto>>builder()
                        .status(HttpStatus.OK.value())
                        .message("Lista de enfermedades recuperada exitosamente")
