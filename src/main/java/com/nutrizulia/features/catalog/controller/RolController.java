...
public class RolController {
    ...
    @GetMapping(CATALOG_ROLES)
    public ResponseEntity<ApiResponseDto<List<RolDto>>> getRoles(HttpServletRequest request) {
        List<RolDto> roles = rolService.getRoles();
        
        ApiResponseDto<List<RolDto>> response = ApiResponseDto.<List<RolDto>>builder()
                .status(HttpStatus.OK.value())
                .message("Lista de roles recuperada exitosamente")
                .data(roles)
